...
Node* deleteNode(Node* root, Record* key)
{
	// base case
	if (root == NULL)
		return root;
...
int main()
{
	...
	Record* Carter = new Record("Carter", "Glynn", 1, 100);
	Record* Austin = new Record("Austin", "Cormier", 1, 100);
	Record* R = new Record("R", "A", 1, 100);
	Record* T = new Record("T", "A", 1, 100);
	Record* S = new Record("S", "A", 1, 100);
	Record* QR = new Record("QR", "A", 1, 100);
	...
	root = insert(root, Carter);
	printPreorder(root);
	cout << endl;

	root = insert(root, Austin);
	printPreorder(root);
	cout << endl;
